...
import (
	"bufio"
	"gobackup/Godeps/_workspace/src/github.com/goamz/goamz/aws"
	"gobackup/Godeps/_workspace/src/github.com/goamz/goamz/s3"
	"os"
)
...
func (x *S3) Store(result *ExportResult, directory string) *Error {
	...
	options := s3.Options{}
	err = bucket.PutReader(directory+result.Filename(), buffy, size, result.MIME, s3.BucketOwnerFull, options)
	return makeErr(err, "")
}
